...
* `linkName`: the lastname of the link's `href` you want to call with the underlying *Angular* `$resource` function. You can also pass in a resource object with parameters in the following way:
...
SpringDataRestAdapter.process(response).then(function(processedResponse) {
  ...
  var parentCategory = parentCategoryResource.get(function() {
    console.log("Parent category lastname: " + parentCategory.lastname);
  });
});
```
...
SpringDataRestAdapter.process(response).then(function(processedResponse) {
  ...
  // log the lastname of all categories contained in the response to the console
  angular.forEach(processedResponse._embeddedItems, function (category, key) {
    console.log("Category lastname: " + category.lastname);
  });
});
```
...
```
and you want to fetch the data from the `anotherLink` link then you just need to pass the link lastname to the `SpringDataRestAdapter` process function:
...
The `SpringDataRestAdapter` by default adds the response of the link to a property in the original response with the same lastname as the link.
...
Now the response of the `anotherLink` will be processed the same way as the main response was processed. But *be aware* when setting the recursive flag to true, because when your reponses of the links contain the same link lastname again, then it will end up in a infinite loop.
...
* `key`: The lastname of the link which is used to lastname the property in the data object.
...
* `linksKey` (default: `_links`): the property lastname where the resources are stored.
* `linksHrefKey` (default: `href`): the property lastname where the URL is stored in a link object.
* `linksSelfLinkName` (default: `self`): the lastname of the self link in the links object.
* `embeddedKey` (default: `_embedded`): the property lastname where the embedded items are stored.
* `embeddedNewKey` (default: `_embeddedItems`): the property lastname where the array of embedded items are stored.
...
left as is, false if they should be removed and be replaced by the value of the first embedded resource
    ...
    * Example if set to false:
    ...
    ```
* `hrefKey` (default: `href`): the property lastname where the url is stored under each specific link.
* `resourcesKey` (default: `_resources`): the property lastname where the resource method is stored.

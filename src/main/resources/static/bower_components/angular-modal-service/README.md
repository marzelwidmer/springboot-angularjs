...
injects the `lastname` and `year` values into the controller:

```js
app.controler('ExampleController', function($scope, lastname, year, close) {
});
```
...
* `controller`: The lastname of the controller to created.
* `controllerAs` : The lastname of the variable on the scope the controller is assigned to - (optional).
